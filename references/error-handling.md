# Error Handling for Krea Nano Banana 2
...
### 400 Bad request
- Invalid payload (e.g., prompt too short, invalid aspect ratio, batch size out of range).
- Action: check that prompt is at least 3 characters, aspect ratio is valid, batch size is 1-4.
...
- Action: simplify the prompt (remove potentially flagged content), try a different aspect ratio, or retry. If repeated, the prompt may be hitting content moderation.
...
- Default timeout is 180 seconds. Some high-res (4K) generations take longer.
- Action: increase timeout with `--timeout 300` (or higher). If it consistently times out, try 1K resolution first to verify the prompt works.
